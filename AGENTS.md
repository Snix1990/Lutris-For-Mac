...
- **Git**: Remote ist `origin` (HTTPS). `opencode` darf (und soll) selbstständig committen und pushen – nicht erst fragen. Vor Push immer `git pull --rebase` (global `pull.rebase true`). Commit-Nachrichten auf Englisch, prägnant.
