...
@MainActor
public final class RuntimeAPI: ObservableObject {
    public static let shared = RuntimeAPI()

    @Published public var isRunning = false
    @Published public var port: UInt16 = 9876
    ...
    private func route(method: String, path: String, body: String) -> Data {
        ...
        case ("GET", "/games"):
            let games = GameRepository.allGames.map { game in
                [
    ...
    public func fireWebhook(gameName: String, playTime: TimeInterval, exitCode: Int32) {
    ...
    public func runPreLaunchScript(_ script: String, gameName: String) {
    ...
    public func runPostExitScript(_ script: String, gameName: String, playTime: TimeInterval) {
        guard !script.isEmpty else { return }
        Task {
            let cmd = script.hasPrefix("#!") ? script : "/bin/bash -c '\(script.replacingOccurrences(of: "'", with: "'\\''"))'"
            _ = try? await ProcessRunner.run(command: cmd, currentDirectory: nil, environment: [
