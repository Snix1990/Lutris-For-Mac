...
@MainActor
public final class WineManager: ObservableObject {
    public static let shared = WineManager()

    @Published public var installedVersions: [WineVersion] = []
    @Published public var prefixes: [WinePrefix] = []
    @Published public var crossOverInstallations: [CrossOverInstallation] = []
    @Published public var isDownloading = false
    @Published public var downloadProgress: Double? = 0
    @Published public var isDownloadingToCache = false
    @Published public var cacheDownloadProgress: Double? = 0
    ...
    public static var allKnownDownloads: [WineDownloadInfo] {
        knownDownloads
    }
    ...
    public var availableDownloads: [WineDownloadInfo] {
        Self.knownDownloads.filter { dl in
            !installedVersions.contains(where: { $0.name == dl.name })
        }
    }

    public var cachedDownloadNames: [String] {
    ...
    public func isCached(_ name: String) -> Bool {
    ...
    public func archivePathInCache(_ name: String) -> URL {
        let safeName = name.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? name
    ...
    public func importCrossOverBottles() {
        let bottlesDirs = [
    ...
    public struct CrossOverGameCandidate {
        public let name: String
        public let exePath: String?
        public let bottleName: String

        public init(name: String, exePath: String?, bottleName: String) {
            self.name = name
    ...
    public func scanCrossOverBottlesForGames() -> [CrossOverGameCandidate] {
    ...
    public func addCrossOverInstallation(path: String) {
    ...
    public func removeCrossOverInstallation(_ installation: CrossOverInstallation) {
        crossOverInstallations.removeAll { $0.id == installation.id }
        saveCrossOverInstallations()
        detectSystemWine()
    }

    public func updateCrossOverInstallation(_ installation: CrossOverInstallation) {
    ...
    public struct WineDownloadInfo: Identifiable {
        public let id = UUID()
        public let name: String
        public let displayName: String
        public let url: URL
        public let type: WineVersion.WineSource

        public init(name: String, displayName: String, url: URL, type: WineVersion.WineSource) {
            self.name = name
    ...
    public func downloadWine(_ info: WineDownloadInfo) async throws {
        isDownloading = true
        downloadProgress = 0
    ...
    public func downloadToCacheOnly(_ info: WineDownloadInfo) async throws {
    ...
    public func removeVersion(_ version: WineVersion) {
    ...
    public func removeCachedArchive(_ name: String) {
    ...
    public func createPrefix(name: String, architecture: WinePrefix.WineArch, wineVersion: WineVersion? = nil) async throws {
    ...
    public func deletePrefix(_ prefix: WinePrefix) {
        try? FileManager.default.removeItem(atPath: prefix.path)
        prefixes.removeAll { $0.id == prefix.id }
        savePrefixes()
    }

    public func runWinecfg(for prefix: WinePrefix, wineVersion: WineVersion? = nil) {
        runWineTool(arguments: ["winecfg"], prefix: prefix, wineVersion: wineVersion)
    }

    public func runRegedit(for prefix: WinePrefix, wineVersion: WineVersion? = nil) {
        runWineTool(arguments: ["regedit"], prefix: prefix, wineVersion: wineVersion)
    }

    public func runWineTool(arguments: [String], prefix: WinePrefix, wineVersion: WineVersion? = nil) {
        let wine = wineVersion ?? installedVersions.first ?? installedVersions.first(where: { $0.wineBinaryPath != "" })
    ...
    public func runWinecfg(prefixPath: String, wineVersion: WineVersion? = nil) {
    ...
    public func runWinecfg(wineBinaryPath: String, prefixPath: String) {
    ...
    public func runRegedit(prefixPath: String, wineVersion: WineVersion? = nil) {
        let tempPrefix = WinePrefix(name: "temp", path: prefixPath, architecture: .win64)
        runWineTool(arguments: ["regedit"], prefix: tempPrefix, wineVersion: wineVersion)
    }

    public func runRegedit(wineBinaryPath: String, prefixPath: String) {
        let version = WineVersion(name: "custom", path: wineBinaryPath, wineBinaryPath: wineBinaryPath, type: .custom)
        let tempPrefix = WinePrefix(name: "temp", path: prefixPath, architecture: .win64)
    ...
    public func detectD3DMetal() -> String? {
        var candidates = [
    ...
    public func gptkDLLDirectory() -> String? {
    ...
    public func deployDLSSDLLs(for game: Game) throws {
    ...
    public func removeDLSSDLLs(for game: Game) {
    ...
    public func prefixForGame(_ game: Game) -> WinePrefix? {
    ...
    public func wineEnvironment(for game: Game, prefix: WinePrefix? = nil, wineVersion: WineVersion? = nil) -> [String: String] {
    ...
    public func launchGame(game: Game) async throws -> String {
    ...
    public func runGame(installPath: String, arguments: String = "", prefix: WinePrefix? = nil, wineVersion: WineVersion? = nil, environment: [String: String] = [:], captureOutput: Bool = true) async throws -> String {
        let wine = wineVersion ?? installedVersions.first ?? installedVersions.first(where: { $0.wineBinaryPath != "" })
        guard let wine, FileManager.default.isExecutableFile(atPath: wine.wineBinaryPath) else {
            throw WineError.noWineVersion
        }
    ...
    public func resolveWineVersion(for wineVersionName: String?) -> WineVersion? {
